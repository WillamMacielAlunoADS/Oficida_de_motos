...
<html lang="en" dir="ltr">
  ...
  <br>
  <div class="container">
    <h3>Pesquisar Peça</h3>
 <form class="form-inline" action="buscar_peca.php" method="post">
  <div class="form-group">
    <input type="text" class="form-control" name="peca" required>
...
<br>
    ...
    include_once '../controllers/conexao.php';
  	include_once '../controllers/PecaDAO.php';

    $peca_b = $_POST;
    $peca = new PecaDAO();

    $resultado = $peca-> buscar_peca_indice($conn, $peca_b);
    echo '
     <div class="container">
     <h3>Clientes</h3><br>
  ...
  if($resultado) {
    while($reseber = $resultado->fetch(PDO::FETCH_OBJ)) {
      echo '<tr>';
      echo 	'<td>'.$reseber -> idpeca .'</td>'.
          '<td>'.$reseber -> nome .'</td>'.
          '<td>'.$reseber -> estoque .'</td>'.
          '<td>'. $reseber -> preco .'</td>
          <td> <a class="btn btn-primary" href="form_alterar_peca.php?id='.
           $reseber -> idpeca.'" role="Controler/button">Alterar</a>'.'</td>
          <td><a class="btn btn-danger" href="../controllers/deletar_peca.php?id='.
           $reseber -> idpeca.'" role="button">Deletar</a></td>';
    echo '</tr>';
    }
   }
      $conn = null;
